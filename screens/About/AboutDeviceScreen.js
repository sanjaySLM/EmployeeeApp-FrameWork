  ...
  function DeviceConstantsTitle({name, brandValue, modelValue}) {
    return (
      <View style={styles.titlecontainer}>
        <Text style={{fontSize: 18}}>
          {' '}
          {typeof value === 'boolean' ? String(brandValue) : brandValue}
        </Text>
        <Text style={{fontSize: 20}}>
          {' '}
          {typeof value === 'boolean' ? String(modelValue) : modelValue}
        </Text>
      </View>
    );
  }
      ...
      <View
        ...
        <View style={{flexDirection: 'row', justifyContent: 'space-between'}}>
          ...
          <Ionicons
            ...
            name={answerShown ? 'chevron-up' : 'chevron-down-outline'}
            size={24}
            color={Colors.primaryColor}
          />
        </View>
        {answerShown ? (
          <Text style={{marginLeft: 30, fontSize: 18}}>{value}</Text>
        ) : null}
      </View>
    );
  }
  return (
    <SafeAreaView>
      <ScrollView contentContainerStyle={{padding: 12}}>
        <DeviceConstantsTitle
          name="Device Brand"
          brandValue={Device.brand}
          modelValue={Device.modelName}
        />
      ...
      /> */}
        ...
        <DeviceMethods
          ...
          method={Device.getMaxMemoryAsync}
        />
        <DeviceMethods

...
import { Alert, Text, View, ActivityIndicator, StyleSheet, ScrollView, TextInput, Picker } from 'react-native';
...
const EditEmployeeScreen = ({ route, navigation, props }) => {
    ...
    if (id) {
        editingProduct = useSelector(state => state.EmployeeList.employeeList.find(emp => emp.id === id))
    }
    ...
    const submitHandler = useCallback(async () => {
        ...
        try {
            if (editingProduct) {
                await dispatch(editEmployee(id, formstate.inputValues.name, formstate.inputValues.gender, +formstate.inputValues.salary, formstate.inputValues.doj))
            } else {
                await dispatch(addEmployee(formstate.inputValues.name, formstate.inputValues.gender, +formstate.inputValues.salary, formstate.inputValues.doj))
            }
            navigation.goBack();
        } catch (err) {
        ...
        <ScrollView>
            <View style={styles.form}>
                ...
                <View style={styles.formControl}>
                    ...
                    {!formstate.inputValidities.gender && !errorText ? <Text>Enter Male or Female only</Text> : null}
                </View>
                {/* <Picker
                ...
                <View style={styles.formControl}>
                    ...
                    <View style={styles.dateContainer}>
                        <View style={styles.datePickerStyle}><PhoenixDateTime onChangeDate={gettingDate} /></View>
                        <View style={styles.dateTextContainer}><Text style={!formstate.inputValues.doj ? styles.selectHereText : styles.dateText}>{formstate.inputValues.doj ? formstate.inputValues.doj : '<= SELECT HERE'}</Text></View>
                    </View>
                </View>
